...
class HubSpotClient:
        ...
        def schedule_messages():
            try:
                ...
                threading.Timer(60, self._send_follow_up_message, 
                              args=(user_id, 'first_follow_up')).start()
                ...
                threading.Timer(120, self._send_follow_up_message, 
                              args=(user_id, 'second_follow_up')).start()
    ...
    def _send_follow_up_message(self, user_id: str, message_type: str):
        """
        THREAD-SAFE отправка follow-up сообщения
        """
        try:
            messages = {
                'first_follow_up': """🎓 Спасибо за интерес к нашим курсам!

Через несколько минут мы свяжемся с вами для уточнения деталей и записи на пробный урок.

Если у вас есть срочные вопросы - пишите прямо здесь! 😊""",
                
                'second_follow_up': """📞 Мы готовы связаться с вами!

Пожалуйста, укажите удобное время для звонка:
• Утром (9:00-12:00)
• Днем (12:00-15:00) 
• Вечером (15:00-18:00)

Или напишите любое удобное для вас время! ⏰"""
            }
