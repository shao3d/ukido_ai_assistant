...
class ProductionIntelligentAnalyzer:
    ...
    def __init__(self):
        ...
        self.fast_keywords = {
            ...
            'off_topic': [
                ...
                'привет', 'здравствуй', 'добрый', 'хай', 'йо', 'салют',
                'пока', 'до свидания', 'увидимся', 'бай',
                # Эмоциональные выражения
                ':)', ':(', ':))', ':((', '😊', '😢', '😂', 'лол', 'ха-ха', 'хехе', 'кек',
                'ого', 'вау', 'круто', 'супер', 'класс', 'уаа', 'ааа', 'чего-чего',
                # Вопросы о состоянии
                'как дела', 'как жизнь', 'что нового', 'как сам', 'как поживаете',
                'что делаешь', 'чем занимаешься',
                # Общие темы не связанные с образованием
                'погода', 'дождь', 'солнце', 'холодно', 'жарко',
                'новости', 'политика', 'экономика', 'война', 'выборы',
                'спорт', 'футбол', 'баскетбол', 'теннис',
                'фильм', 'кино', 'сериал', 'музыка', 'песня',
                'еда', 'готовить', 'рецепт', 'ресторан',
                'работа', 'офис', 'зарплата', 'начальник',
                # Короткие реакции
                'ок', 'окей', 'хорошо', 'понятно', 'ясно', 'спасибо', 'благодарю',
                'да', 'нет', 'может быть', 'не знаю',
                # Бессмысленные сообщения
                'тест', 'проверка', 'алло', 'есть кто', 'слышно',
                'абракадабра', 'бла-бла', 'ля-ля'
            ]
        }
    ...
    def enrich_query_with_context(self, query: str, conversation_history: List[str] = None) -> str:
        ...
        if any(word in query.lower() for word in context_indicators):
            ...
            return query
        # Улучшенное обогащение для общих запросов
        query_enrichment_map = {
            # Общие вопросы о школе
            'школ': 'школа Ukido онлайн курсы soft skills развитие навыков детей',
            'расскажи': 'информация о школе Ukido курсы программы обучение методика Анна Коваленко',
            'что такое': 'описание школы Ukido обучение навыки soft skills',
            'как работает': 'методика Ukido процесс обучения занятия структура',
            'ваш': 'школа Ukido наша методика подход программы',
            'укидо': 'Ukido школа soft skills развитие навыков',
            # Курсы и программы
            'курс': 'курсы программы Ukido обучение детей возрастные группы Юный Оратор Эмоциональный Компас Капитан Проектов',
            'программа': 'программы Ukido курсы структура обучения',
            'урок': 'уроки занятия Ukido структура формат пробный урок',
            'занятия': 'занятия уроки Ukido расписание формат',
            # Преподаватели и методика
            'учитель': 'преподаватели Ukido квалификация опыт команда Анна Коваленко',
            'преподаватель': 'преподаватели Ukido учителя команда Анна Коваленко',
            'методика': 'методика Ukido подход обучение soft skills',
            'подход': 'подход методика Ukido философия обучения',
            # Результаты и польза
            'результат': 'результаты Ukido эффективность достижения навыки',
            'польза': 'польза результаты эффективность Ukido развитие',
            'навыки': 'soft skills навыки Ukido развитие коммуникация',
            'развитие': 'развитие навыков Ukido soft skills личностный рост',
            # Практические вопросы
            'как записаться': 'запись регистрация Ukido как попасть процедура пробный урок',
            'как попасть': 'поступление запись Ukido требования процедура',
            'стоимость': 'цены стоимость Ukido оплата тарифы',
            'цена': 'цены стоимость оплата Ukido курсы',
            # Возрастные группы
            'возраст': 'возрастные группы Ukido дети программы курсы 7-10 лет 9-12 лет 11-14 лет',
            'ребенок': 'дети возраст Ukido программы развитие',
            'лет': 'возраст лет дети Ukido группы программы',
            # Формат обучения
            'онлайн': 'онлайн обучение Ukido дистанционно интернет',
            'формат': 'формат обучения Ukido онлайн занятия структура',
            'расписание': 'расписание время Ukido занятия график'
        }
